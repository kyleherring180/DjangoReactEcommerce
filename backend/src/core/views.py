...
import stripe
stripe.api_key = 'sk_test_b2hRiuLosD8WtaJJoK15D3bI00RFKbVIie'
    ...
    def get(self, *args, **kwargs):
        try:
            ...
            if shipping_address_qs.exists():
                ...
                context.update({'use_default_shipping':use_default_shipping})
            ...
            #     context.update({'use_default_billing':use_default_billing})
        ...
        try:
            ...
            if form.is_valid():
                ...
                if use_default_shipping:
                    ...
                    if is_valid_form([shipping_address1, shipping_country, shipping_region, shipping_city, shipping_zip_code]):

                        shipping_address = Address(
                            user = self.request.user,
                            street_address = shipping_address1,
                            street_address_2 = shipping_address2,
                ...
                if same_billing_address:
                    ...
                    if address_qs.exists():
                        ...
                        messages.info(self.request, "No default billing address was found!")
                        return redirect('core:checkout')
                else:
                    ...
                    if is_valid_form([billing_address1, billing_country, billing_region, billing_city, billing_zip_code]):

                        billing_address = Address(
                            user = self.request.user,
                            street_address = billing_address1,
                            street_address_2 = billing_address2,
                ...
                #add logic that checks if get_total was 0 after coupon was applied payment was successful
                if not order.get_total():
                    payment = Payment()
                    payment.user = self.request.user
                    payment.amount = order.get_total()
                    payment.city = order.billing_address.city
...
class PaymentView(View):
    def get(self, *args, **kwargs):
        ...
        if order.billing_address:
            ...
            messages.warning(self.request, "Please complete your billing details!")
            return redirect("core:checkout")
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            # create the payment
            payment = Payment()
            payment.stripe_charge_id = uuid.uuid1() #charge['id']
            payment.user = self.request.user
            payment.amount = order.get_total()
            payment.city = order.billing_address.city
            if order.coupon:
            ...
            try:
                ...
                if coupon:
                    ...
                    if order.coupon.used < order.coupon.limit:
                        ...
                        messages.warning(self.request, "Coupon is no longer valid!")
                        return redirect("core:checkout")
                else:
                    messages.warning(self.request, "This coupon is invalid")
                    return redirect("core:checkout")
